...
export const GirlfriendBoyfriend: React.FC = () => {
  ...
  const physicalItems = SAMPLE_PRODUCTS.filter(
    (p) => !p.isEGift && p.category === 'girlfriend-boyfriend' && p.subCategory === activeTab
  );
  ...
  const isGirlfriend = activeTab === 'girlfriend';
  ...
  return (
    <div className="space-y-12">
      ...
      <section className={`text-center py-12 md:py-16 rounded-3xl border transition-all duration-300 relative overflow-hidden ${
        ...
        <div className="max-w-xl mx-auto px-4 relative z-10 space-y-4">
          <span className={`text-[10px] uppercase font-mono tracking-widest font-extrabold px-3 py-1.5 rounded-full ${
            isGirlfriend ? 'bg-brand-pink-soft text-brand-pink' : 'bg-brand-blue-soft text-brand-blue'
          }`}>
            Relationship Repair Centre 💌
          </span>
          ...
          <p className="text-xs md:text-sm text-gray-500 font-medium leading-relaxed">
            Reconnection starts with a warm gesture. Find cute physical frames & plush rose boxes, or instant cute love letter slideshows for them.
          </p>
          ...
          <div className="inline-flex p-1 bg-white rounded-2xl border border-gray-100 shadow-sm max-w-xs mx-auto">
            ...
            >
              <Heart size={12} className={isGirlfriend ? "fill-white text-white" : ""} />
                    ...
                    <div className="absolute top-3 left-3 flex gap-1">
                      {product.isEGift ? (
                        ...
                        <span className={`text-[9px] font-mono uppercase font-extrabold px-2 py-0.5 rounded-lg border ${
                          isGirlfriend ? 'bg-brand-pink-soft text-brand-pink border-brand-pink-soft' : 'bg-brand-blue-soft text-brand-blue border-brand-blue-soft'
                        }`}>Physical Gift</span>
                      )}
                    </div>
                  </div>
      ...
      {/* Helpful communication advice callout banner */}
      <section className="bg-gradient-to-r from-brand-blue-soft/20 via-white to-brand-pink-soft/15 rounded-3xl p-6 border border-brand-blue-soft flex flex-col sm:flex-row items-center gap-5 justify-between">
        <div className="flex items-center gap-4 text-center sm:text-left">
          <div className="p-4 bg-brand-blue-soft text-brand-blue rounded-2xl text-2xl">💌</div>
          <div>
            <h4 className="font-display font-bold text-gray-800 text-sm">Need a specialized apology strategy layout?</h4>
            <p className="text-xs text-gray-500 mt-0.5 font-medium leading-relaxed">Our relationship counselors can assist you in choosing the exact flow of texts and cards over WhatsApp to suit their taste. Tap here to chat!</p>
          </div>
        </div>
        <button
